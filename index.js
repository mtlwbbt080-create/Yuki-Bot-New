...
// --- 🎯 بنك الأسئلة ومقولات الأنمي الضخم والمضخم جداً (أكثر من 40 سؤال بالاختيارات) 🎯 ---
const animeQuizzes = [
...
async function startBot() {
    // كود الأمان لـ Render لمنع التعليق وتطهير الجلسات المعطلة
    if (fs.existsSync('./session_yuki/creds.json')) {
        try {
            const creds = fs.readJsonSync('./session_yuki/creds.json');
            if (!creds.me) { 
                console.log('🗑️ [يوكي] تم اكتشاف كاش معطل، جاري حذفه لطباعة الـ QR...');
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // --- نظام الفحص الذكي للإجابات (الصح والغلط بالأرقام والأسماء) ---
