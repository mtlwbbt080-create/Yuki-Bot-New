...
const qrcode = require('qrcode-terminal'); // المكتبة الأساسية لطباعة الـ QR الفخم
...
async function startBot() {
    // إجبار السيرفر على إزالة الـ Session القديمة لضمان ظهور الـ QR الجديد فوراً
    ...
    const sock = makeWASocket({ 
        ...
        printQRInTerminal: false // نجعل qrcode-terminal تتولى الطباعة بنسق متوافق
    });
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        // 🌟 هنا يكمن الحل! استقبال مصفوفة الـ QR وطباعتها بحجم ملموم ومناسب لـ Render
        if (qr) {
            console.log('\n==================================================');
            console.log('🌸 امسح الـ QR كود التالي لتشغيل إمبراطورية يوكي 🌸');
            console.log('==================================================\n');
            qrcode.generate(qr, { small: true }); // خيار small يمنع تمدد وتشوه الرمز داخل السجلات
            console.log('\n==================================================\n');
        }
        ...
        switch (cmd) {
            case 'اوامر':
                const menu = `🌸 *مرحباً بك في عالم يوكي السحري المطور* 🌸\n\n` +
                             ...
                             `🎲 *تسلية وحب وحماس:* (.روليت، .لوخيروك Reds، .كت تويت، .نسبة_الحب)\n` +
